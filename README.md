...
- If you experience issues during the startup consider to click `File/Invalidate Caches/restart`
 ...
 ### How to use
 - Open [Postman](https://www.getpostman.com/), call `auth/signin` endpoint by a valid credential e.g. `username: test, password: test` and save the token to the environment variables in postman as `token`which is generated by the server
